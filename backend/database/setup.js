...
const setupDatabase = async () => {
  ...
  try {
    ...
    await connection.execute(
      `CREATE DATABASE IF NOT EXISTS ${process.env.DB_NAME || 'local_store_db'}`
    );
...
const insertSampleData = async (connection) => {
  try {
    ...
    for (const [name, description, price, sale_price, category_id, stock, image] of products) {
      await connection.execute(
        'INSERT INTO products (name, description, price, sale_price, category_id, stock_quantity, image_url) VALUES (?, ?, ?, ?, ?, ?, ?)',
